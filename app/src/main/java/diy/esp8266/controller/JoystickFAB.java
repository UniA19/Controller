                    ...
                    switch (event.getActionMasked()) {
                        ...
                        case MotionEvent.ACTION_MOVE:
                            if (Math.sqrt(Math.pow(event.getRawX() - posX, 2) + Math.pow(event.getRawY() - posY, 2)) < RADIUS) {
                                ...
                                if (position.equals("left")) {
                                    Connection.setLeft(Math.round(((event.getRawX() - posX) * temp) * 100 / RADIUS), Math.round(((event.getRawY() - posY) * temp)) * 100 / RADIUS * -1);
                                } else {
                                    Connection.setRight(Math.round(((event.getRawX() - posX) * temp) * 100 / RADIUS), Math.round(((event.getRawY() - posY) * temp)) * 100 / RADIUS * -1);
                                }
                            }
                            break;
